...
void PBRIsoScene::CompileLightingShader()
{
    ...
    "   vec2 rVec = 2*texture2D(map_noise, gl_TexCoord[0].xy).rg-1.0;"
    ...
    "       vec3 groundColor = texture2DLod(map_albedo, groundPos*view_ratio,materialPixel.r*20.0).rgb*10.0/(1.0+length(v)+heightPixel);"
    ...
    "   gl_FragColor.rgb = (albedoPixel.rgb  * kDAmbient* irradianceAmbient + specularAmbient); "
    ""
    ""
    "   int curShadowMap = 0;"
	...
	"	    gl_FragColor.rgb += (kD * albedoPixel.rgb *"<<1.0/PI<<" + specular) * radiance * NdotL;"
	"       float t = materialPixel.b;"
	"	    gl_FragColor.rgb -= (albedoPixel.rgb*"<<1.0/PI<<") * radiance * min(dot(direction, lightDirection), 0.0)*t;"
	""
	"   }"
