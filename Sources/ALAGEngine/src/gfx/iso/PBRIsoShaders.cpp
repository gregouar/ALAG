...
void PBRIsoScene::CompileWaterGeometryShader()
{
   ...
   // "       x = 1.0 - x;"
    ...
    "}"
    ""
    "WaveParticle ComputeWave(float x)"
    "{"
    "   x = x - floor(x);"
    ...
    "   WaveParticle r= Quadratic(t,wl/(wl+wr),(middle_point+left_point)*0.5,"
    "                     middle_point,"
    ...
    "   vec3 noise = texture2D(map_noise, VaryingTexCoord0);"
    "   vec3 noiseWave = texture2D(map_noise, VaryingTexCoord0+vec2(p_wave_pos,0)*2);"
    "   WaveParticle wave = ComputeWave((-sin(normalizedCoord.y*"<<PI<<")*0.1 + Coord0.x+p_wave_pos+noiseWave.z*p_wave_turbulence)*p_wave_frequency);"
    "   float waveAttenuation = clamp(sin(normalizedCoord.y*"<<PI<<"),0,1.0);"
   ...
   "     float foam = wave.values.w;" //NEED TO USE A TEXTURE HERE
   //"     foam = smoothstep(.1,.3,wave.z*n.x);"
    ...
    "   velocity.z += p_turbulences_amplitude * 0.2 /* * length(noise.xy) */ ;"
