...
void PBRIsoScene::CompilePBRGeometryShader()
{
  ...
  //  "uniform sampler2D map_opaqueNormal;"
 ...
 //   "   float delta = last.z - cur.z;"
   ...
   // "   p -= view_direction.xy / view_direction.z *  "<<1.0/NBR_PARALLAX_STEPS<<" * total_height * weight;"
    ...
    "   float screen_height = total_height * p_isoToCartZFactor;"
    "   vec2 shift = (vec3(0,screen_height,0)*p_cartToIso2DProjMat).xy/texture_size;"
    ...
    "       p -= shift * "<<1.0/NBR_PARALLAX_STEPS<<";"
    "       depthPixel = texture2D(map_depth, coord - p).rgb;"
    "       if(p_isRepeated == false)"
    "           depthPixel *= insideBox(coord - p,vec2(0.0,0.0),vec2(1.0,1.0));"
    "       oldHeight = curHeight;"
    ...
    "   float weight = (curHeight - layerHeight)/(1.0+delta*"<<NBR_PARALLAX_STEPS<<");"
   ...
   "    p+= shift * weight;"
    ...
    "       if(p_isRepeated == false && insideBox(texCoord,vec2(0.0,0.0),vec2(1.0,1.0)) == 0.0) discard;"
    "   }"
    ""
   ...
   // "           if(velocity.x <= 0) {"
    "           vec2 collisionTest = gl_FragCoord.xy + (velocity * p_isoToCartMat).xy * vec2(1.0,-1.0);"
    ...
    "               p.z = abs(p.z);"
    "               if(p.z < 0.001){"
    //"                   vec3 slope = texture2D(map_opaqueNormal, p*view_ratio).rgb;"
   // "                   slope = slope*2-1;"
    ...
    "                   foamLevel = length(velocity)/ max(1.0, 5.0*(length(p.xy - gl_FragCoord.xy)))  ;"
    ""
  ...
  //  "           density *= smoothstep(.5,1.0,clamp(foamLevel,.5,1.0)) * 2.0;"
   ...
   //"            albedoPixel = vec4(velocityPixel,.9);"
    ...
    "           float depth = exp(-(depthTest-zPixel)*density);"
    "           albedoPixel.a = mix(albedoPixel.a, 0.0, smoothstep(0.0,1.0,clamp(depth,0.0,1.0)));"
...
void PBRIsoScene::CompileWaterGeometryShader()
{
   ...
   // "uniform vec2 texture_size;"
    ...
    //"uniform vec2 p_wave_velocity["<<IsoWaterEntity::NBR_WAVE_SHAPE_SAMPLES<<"];"
    "uniform float p_wave_xDistribution["<<IsoWaterEntity::NBR_WAVE_SHAPE_SAMPLES<<"];" \
   ...
   // "   if(x > .5)"
   // "       x = 1.0 - x;"
    "    if(x < p_wave_xDistribution[0] || x > p_wave_xDistribution["<<IsoWaterEntity::NBR_WAVE_SHAPE_SAMPLES-1<<"]) return -1;"
    "   int cur = 0;"
    "   float cur_pos = p_wave_xDistribution[1]*0.5;"
    "   while(cur < "<<IsoWaterEntity::NBR_WAVE_SHAPE_SAMPLES-1<<" && x > cur_pos){"
    "       cur++;"
    "       if(cur < "<<IsoWaterEntity::NBR_WAVE_SHAPE_SAMPLES-1<<") {"
    "       cur_pos += (p_wave_xDistribution[cur] - p_wave_xDistribution[cur-1])*0.5; "
    "       cur_pos += (p_wave_xDistribution[mod(cur+1,"<<IsoWaterEntity::NBR_WAVE_SHAPE_SAMPLES<<")] - p_wave_xDistribution[cur])*0.5;"
    "       } "
    "   }"
    "   return cur;"
    "}"
    ""
    "WaveParticle Quadratic(float x, float PX, vec4 yl, vec4 P, vec4 yr)"
    "{"
    "   vec4 a = (yr - yl - (P - yl)/PX);"
    "   vec4 b = (P-yl)/PX;"
    "   vec4 r = a*x*x + b*x + yl;"
    "   float DX = 2*a.x*x + b.x;"
    "   return WaveParticle(DX,r);"
    "}"
    ""
    "WaveParticle ComputeGerstnerWave(float x)"
    "{"
    "   x = x - floor(x) + 0.05;"
    "   int c = SearchWavePos(x);"
    "   WaveParticle r;"
    "   r.values = vec4(-1,0,0,0);"
    "   r.DX = 0;"
    "   if(c == -1) return r;"
    "   vec4 left_point = p_wave_shape[mod(c-1,"<<IsoWaterEntity::NBR_WAVE_SHAPE_SAMPLES<<")];"
    "   float left_pointX = p_wave_xDistribution[mod(c-1,"<<IsoWaterEntity::NBR_WAVE_SHAPE_SAMPLES<<")];"
    //"   if(c-1 == -1) {left_point = p_wave_shape[1]; left_pointX = -p_wave_xDistribution[1];}"
    "   vec4 middle_point = p_wave_shape[c];"
    "   float middle_pointX = p_wave_xDistribution[c];"
    "   vec4 right_point = p_wave_shape[mod(c+1,"<<IsoWaterEntity::NBR_WAVE_SHAPE_SAMPLES<<")];"
    "   float right_pointX = p_wave_xDistribution[mod(c+1,"<<IsoWaterEntity::NBR_WAVE_SHAPE_SAMPLES<<")];"
    //"   if(c+1 == "<<IsoWaterEntity::NBR_WAVE_SHAPE_SAMPLES<<") right_pointX = 1.0 + p_wave_xDistribution[1];"
    "   float wr = (right_pointX - middle_pointX)*0.5;"
    "   float wl = (middle_pointX - left_pointX)*0.5;"
    "   float t = (x - left_pointX - wl)/(wl+wr);"
    "   r= Quadratic(t,wl/(wl+wr),(middle_point+left_point)*0.5,"
    "                     middle_point,"
    "                   (right_point+middle_point)*0.5);"
    "   r.DX /= wl+wr;"
    "   return r;"
    "}"
    ""
    "WaveParticle ComputeSinWave(float x)"
    "{"
    "   WaveParticle w;"
    "   w.values.x = sin(x*"<<2* PI<<");"
    "   w.values.y = .3;" //VELOCITY
    "   w.values.z = -1;" //VELOCITY
    "   w.values.w = 0.0;"
    "   w.DX = -cos(x*"<<2* PI<<");"
    "   return w;"
    "}"
    ...
    "   vec3 noiseWave = texture2D(map_noise, VaryingTexCoord0+vec2(p_wave_pos,0)*.5);"
    "   float wX = ( /*-sin(normalizedCoord.y*"<<PI<<")*0.1 */ + Coord0.x /*+p_wave_pos*/+noiseWave.z*p_wave_turbulence);"
    "   WaveParticle gWave = ComputeGerstnerWave(1.0-wX);"
    "   WaveParticle sWave = ComputeSinWave((.5- Coord0.x*p_wave_frequency-p_wave_pos));"
    "   sWave.values = vec4(0,-1,-1,0);"
    "   float waveAttenuation = clamp(sin(normalizedCoord.y*"<<PI<<"),0.0,1.0);"
    "   WaveParticle wave;"
    "   wave.values = mix(sWave.values, gWave.values,waveAttenuation);"
    "   wave.DX = mix(sWave.DX, gWave.DX,waveAttenuation);"
    //"   wave.values *= waveAttenuation;"
    //"   wave.DX *= waveAttenuation;"
    "   "
    "   vec3 noise = texture2D(map_noise, VaryingTexCoord0 /*+ vec2(wave.values.y,0)*.05*/ /*-vec2(p_wave_pos,0)*0.25*/);"
    //"   noise = vec3(1.0) - noise;"
    "   noise.xy *= 0.5;"
    ""
    "   vec3 water = vec3(-wave.DX/(40.0 /* *p_wave_frequency */),0.0,wave.values.x) * p_wave_amplitude + noise * p_turbulences_amplitude;"
    "   water.z = 0.5 + water.z * 0.5;"
   "     float foam = wave.values.w   * p_wave_amplitude  /* * pow(waveAttenuation,2)*/;" //NEED TO USE A TEXTURE HERE
   //"    water.z += foam*noiseWave.z*0.5;"
   "    foam *= 0.7+(noiseWave.z*3.0-floor(noiseWave.z*3.0))*0.3;"
   "     foam = smoothstep(.0,.8,foam);"
   "    water.z += foam * 0.1 * p_wave_amplitude;"
   //"    water.xy += foam * noiseWave.xy;"
    ...
    "    gl_FragData["<<PBRMaterialScreen<<"] = mix(p_waterMaterial, vec4(.5, 0.0,.1,.0),foam);"
    "   vec3 velocity = p_wave_amplitude * vec3(wave.values.y,noise.z, wave.values.z) * (0.7+(noise.z*2.0-floor(noise.z*2.0))*0.3);"
    "   velocity.z -= p_turbulences_amplitude * 5 * (.5+noise.z*.25) /* * length(noise.xy) */ ;"
    "   velocity.xy += p_turbulences_amplitude * noise.xy  * (0.7+noise.z*0.3);"
