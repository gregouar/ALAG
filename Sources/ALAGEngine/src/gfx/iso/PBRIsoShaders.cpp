...
void PBRIsoScene::CompilePBRGeometryShader()
{
  ...
  /*  "uniform sampler2D map_depthTester;" \
   ...
   // "                          -depth*"<< PBRTextureAsset::DEPTH_BUFFER_NORMALISER_INV <<";"
    ...
    "           vec3 velocity = ( velocityPixel * 2.0 - 1.0)*30.0;"
    "           velocity.x = clamp(0.0,30.0,velocity.x);"
    ...
    "                   float foamLevel = smoothstep(0.0,1.0,length(p.xy - gl_FragCoord.xy)*"<<1.0/30.0<<");"
    //"                   float foamLevel = length(p.xy - gl_FragCoord.xy)*"<<1.0/30.0<<";"
    "                   albedoPixel = mix(vec4(.8,.8,.8,.8), albedoPixel,foamLevel);"
    "                   materialPixel = mix(vec4(.5, 0.0,.1,1.0), materialPixel,foamLevel);"
    "                   density = mix(.01,density, foamLevel);"
    "               }"
    "           }"
    "       }"
    ""
...
void PBRIsoScene::CompileWaterGeometryShader()
{
   ...
   "     float foam =  smoothstep(.75,1.0,water.z);" //NEED TO USE A TEXTURE HERE
   ...
   //"     foam = clamp(foam+smoothstep(.95,1.0,water.w),0.0,.8);"
    ...
    "    gl_FragData["<<PBRExtraScreen0<<"] = vec4(0.5+normalize(vec3(wave.z,0.0, wave.w* p_wave_amplitude))*0.5,1.0);"
    "}";
