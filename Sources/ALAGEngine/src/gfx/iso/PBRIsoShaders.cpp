...
void PBRIsoScene::CompileLightingShader()
{
    ...
    "       vec3 groundColor = texture2D(map_albedo, groundPos*view_ratio).rgb*10.0/(length(v)+heightPixel);"
	...
	"                      +texture2D(map_SSAO, gl_TexCoord[0].xy+(vec2(0,-1))*view_ratio).b)/6.0;"
	"       if(occlusion > .2 && occlusion < .8) {"
    "           occlusion = 3.0 * occlusion/4.0"
