...
void PBRIsoScene::CompileLightingShader()
{
   ...
   // "uniform float p_exposure;"
    ...
    "}"
    "vec2 RaySearch(vec3 start, vec3 end)"
    "{"
    "   vec3 cur = start;"
    "   vec3 diff = end-start;"
    "   float step = 0.5;"
    ...
    "   for(int i = 0 ; i < 32 ; ++i) {"
    "       curScreenPos = oldScreenPos + 10*(v*p_isoToCartMat).xy * constantList.xz;"
    "       curWorldPos  = oldWorldPos + 10*v;"
    "       float heightPixel = GetDepth(curScreenPos);"
    "       if(heightPixel - curWorldPos.z > 0 /*&& heightPixel - curWorldPos.z < 100*/){"
    "           curScreenPos = RaySearch(vec3(oldScreenPos, oldWorldPos.z), vec3(curScreenPos, curWorldPos.z));"
    "           r.xy = curScreenPos;"
    "           r.z = 1.0;"
    /*"           if(GetDepth(curScreenPos+vec2(2,0)) > curWorldPos.z)"
    "               r.z += 0.2;"
    "           if(GetDepth(curScreenPos+vec2(0,2)) > curWorldPos.z)"
    "               r.z += 0.2;"
    "           if(GetDepth(curScreenPos+vec2(-2,0)) > curWorldPos.z)"
    "               r.z += 0.2;"
    "           if(GetDepth(curScreenPos+vec2(0,-2)) > curWorldPos.z)"
 ...
 //   "   vec3 viewDirection = view_direction;"
  //  "   vec3 viewDirection = vec3("<<cos(m_viewAngle.xyAngle*PI/180)*cos(m_viewAngle.zAngle*PI/180)<<","
    ...
    "   reflectionView += mix(vec3(0.0),rVec,materialPixel.r*.8);"
    "   vec2 uv = SampleSphericalMap(normalize(reflectionView));"
    "   vec3 reflectionColor = texture2D(map_environmental, uv).rgb;"
    //"   gl_FragData[1] = vec4(0.5+normalize(reflectionView)*0.5,1.0);"
   // "   reflectionColor = ambientLighting;"
    "   if(enable_SSR == true) {"
    "       vec3 envPos = RayTrace(fragPos,reflectionView);"
    "       if(envPos.z != -1.0){"
    "           vec3 SSRColor = texture2DLod(map_SSRenv, envPos.xy*view_ratio,materialPixel.r*3.0).rgb;"
