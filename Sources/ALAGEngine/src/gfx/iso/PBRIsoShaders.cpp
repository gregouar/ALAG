...
void PBRIsoScene::CompilePBRGeometryShader()
{
 ...
 //   "   float delta = last.z - cur.z;"
   ...
   "    p+= shift * weight;"
    ...
    "           float delta  =  100 /* + abs(Hash(gl_FragCoord).z) * 10.0*/;"
...
void PBRIsoScene::CompileWaterGeometryShader()
{
   ...
   //"    water.xy += foam * noiseWave.xy;"
    ...
    "   vec3 velocity = vec3(wave.values.y,noise.y, wave.values.z) * (0.7+(noise.z*3.0-floor(noise.z*3.0))*0.3);"
    "   velocity.z -= p_turbulences_amplitude * 0.2 * (.5+noise.z*.25) /* * length(noise.xy) */ ;"
