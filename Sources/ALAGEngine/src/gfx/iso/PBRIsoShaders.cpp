...
void PBRIsoScene::CompilePBRGeometryShader()
{
  ...
  /*  "uniform sampler2D map_depthTester;" \
    ...
    "           float depth = (depthTest-zPixel)*p_density;"
...
void PBRIsoScene::CompileWaterGeometryShader()
{
   ...
   // "       x = 1.0 - x;"
    ...
    "   while(cur < 15 && x > cur_pos){"
    "       cur++;"
    "       if(cur < 15) {"
    "       cur_pos += (p_wave_shape[cur].x - p_wave_shape[cur-1].x)*0.5; "
    "       cur_pos += (p_wave_shape[mod(cur+1,16)].x - p_wave_shape[cur].x)*0.5;"
    "       } "
    "   }"
    "   return cur;"
    "}"
    ""
    "vec3 Quadratic(float x, float yl, vec2 P, float yr)"
    "{"
    "   vec3 r = vec3(0.0);"
   /* "   float t;"
    "   if(P.x == 0.5) t = x;"
    "   else if(P.x < 0.5)"
    "       t = (-P.x + sqrt(P.x*P.x + (1.0-2.0*P.x)*x))/(1.0-2.0*P.x);"
    "   else if(P.x > 0.5)"
    "       t = (-P.x + sqrt(P.x*P.x + (1.0-2.0*P.x)*x))/(1.0-2.0*P.x);"
    "   r.z = (1-t*t)*yl + 2*(1-t)*t*P.y + t*t*yr;"
    "   return r;"*/
   /* "   r.z = yl * (1.0 - x) + yr * x;"
    "   r.xy = vec2(yr-yl,0.0);"
    "   return r;"*/
    "   float a = (yr - yl - (P.y - yl)/P.x);"
    "   float b = (P.y-yl)/P.x;"
    "   r.z = a*x*x + b*x + yl;"
    "   r.x = 2*a*x + b;"
    "   return r;"
    "}"
    ""
    "vec3 ComputeWave(float x)"
    "{"
    "   x = x - floor(x);"
    "   int c = SearchWavePos(x);"
    "   vec2 left_point = p_wave_shape[mod(c-1,16)];"
    "   if(c-1 == -1) left_point = p_wave_shape[1]*constantList.zx;"
    "   vec2 middle_point = p_wave_shape[c];"
    "   vec2 right_point = p_wave_shape[mod(c+1,16)];"
    "   if(c+1 == 16) right_point = vec2(1.0,0.0) + p_wave_shape[1];"
    "   float wr = (right_point.x - middle_point.x)*0.5;"
    "   float wl = (middle_point.x - left_point.x)*0.5;"
    "   float t = (x - left_point.x - wl)/(wl+wr);"
    //"   return vec3(float(c)/16);"
    "   return Quadratic(t,(middle_point.y+left_point.y)*0.5,"
    "                    vec2(wl/(wl+wr), middle_point.y),"
    "                   (right_point.y+middle_point.y)*0.5);"
    //"   return vec3(t);"
   // "   return Bezier(x,0,vec2(.1,.9),1);"
    "}"
    ""
    "void main()" \
    "{" \
   ...
   // "   vec3 noise = PerlinNoise(normalizedCoord);"
    ...
    "   vec3 wave = ComputeWave((Coord0.x+p_wave_pos+noise.x*p_wave_turbulence)*p_wave_frequency);"
