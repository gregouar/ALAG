...
void PBRIsoScene::CompilePBRGeometryShader()
{
 ...
 //   "   float delta = last.z - cur.z;"
   ...
   // "                   slope = slope*2-1;"
    ...
    "                   foamLevel = smoothstep(.1,1.0,foamLevel);"
    ""
...
void PBRIsoScene::CompileWaterGeometryShader()
{
   ...
   // "       x = 1.0 - x;"
    ...
    "   noise.xy *= 0.5;"
    ""
    "   vec3 water = vec3(-wave.DX/(20.0*p_wave_frequency),0.0,wave.values.x) * p_wave_amplitude + noise * p_turbulences_amplitude;"
   ...
   "    foam *= 0.75+(noiseWave.z*3.0-floor(noiseWave.z*3.0))*0.25;"
   ...
   //"    water.xy += foam * noiseWave.xy;"
    ...
    "   vec3 velocity = vec3(wave.values.y,noise.z*.5, wave.values.z) * (.5+noise.z*.5) /* * p_wave_amplitude */;"
