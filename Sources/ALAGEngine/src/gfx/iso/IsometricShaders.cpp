...
const std::string lighting_fragShader = \
	...
	"   }"
    "   if(useSSAO == true) {"
	"       float occlusion  = (texture2D(SSAOMap, (gl_FragCoord+vec2(0,0))*screen_ratio).b*4"
	"                      +texture2D(SSAOMap, (gl_FragCoord+vec2(1,0))*screen_ratio).b*2"
	"                      +texture2D(SSAOMap, (gl_FragCoord+vec2(-1,0))*screen_ratio).b*2"
	"                      +texture2D(SSAOMap, (gl_FragCoord+vec2(0,2))*screen_ratio).b*2"
	"                      +texture2D(SSAOMap, (gl_FragCoord+vec2(0,-2))*screen_ratio).b*2"
	"                      +texture2D(SSAOMap, (gl_FragCoord+vec2(1,2))*screen_ratio).b*1"
	"                      +texture2D(SSAOMap, (gl_FragCoord+vec2(1,-2))*screen_ratio).b*1"
	"                      +texture2D(SSAOMap, (gl_FragCoord+vec2(-1,-2))*screen_ratio).b*1"
	"                      +texture2D(SSAOMap, (gl_FragCoord+vec2(1,-2))*screen_ratio).b*1"
	"                       )/16.0;"
